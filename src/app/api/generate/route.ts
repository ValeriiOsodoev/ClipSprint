...
function errorResponse(code: ApiError["error"]["code"], message: string, status: number) {
...
export async function POST(request: NextRequest) {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a YouTube script expert. Output ONLY valid JSON, no markdown or explanation.",
          },
          {
            role: "user",
            content: prompt,
  ...
  if (llmResponse.status === 429) {
    return errorResponse("RATE_LIMIT", "Rate limit exceeded. Please try again later.", 429);
  }
  ...
  if (!llmResponse.ok) {
    ...
    try {
      const errBody = (await llmResponse.json()) as { error?: { message?: string } };
  ...
  const responseResult = generateResponseSchema.safeParse(parsedContent);
  ...
  const responseData: GenerateResponse = responseResult.data;
